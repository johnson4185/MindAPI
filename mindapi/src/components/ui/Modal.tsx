...
export default function Modal({ open, onClose, title, children, footer, maxWidth = 500 }: ModalProps) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div className="modal-animate" onClick={e => e.stopPropagation()} style={{ background: "var(--c-surface)", borderRadius: "var(--r-xl)", boxShadow: "var(--sh-lg)", width: "100%", maxWidth, border: "1.5px solid var(--c-border)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 22px", borderBottom: "1px solid var(--c-border)" }}>
          ...
          <button onClick={onClose} style={{ width: 28, height: 28, borderRadius: 0, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--c-bg)", border: "1.5px solid var(--c-border)", color: "var(--c-ink3)" }}>
